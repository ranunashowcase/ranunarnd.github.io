...
export default function Header({ toggleSidebar, isSidebarOpen, isMobile }: { toggleSidebar?: () => void; isSidebarOpen?: boolean; isMobile?: boolean }) {
  ...
  return (
    <header className={cn(
      "fixed top-0 right-0 h-16 bg-white/80 backdrop-blur-md border-b border-gray-100 z-40 flex items-center justify-between md:justify-end px-4 sm:px-6 transition-all duration-300",
      isSidebarOpen && !isMobile ? "left-[260px]" : "left-0"
    )}>
      
      {/* Sidebar Toggle (Left side on Mobile, or Left side on Desktop when sidebar is closed) */}
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          onClick={toggleSidebar}
          className="hidden md:flex p-2.5 bg-gray-50 text-gray-700 hover:bg-brand-primary/10 hover:text-brand-primary rounded-xl transition-colors border border-gray-100 shadow-sm items-center justify-center"
          aria-label="Toggle Sidebar"
        >
          {isSidebarOpen ? <X className="w-5 h-5" /> : <Menu className="w-5 h-5" />}
        </button>
